      ...
      <form onSubmit={handleSubmit(onSaveItem)}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 border-b border-gray-200 dark:border-gray-700 pb-6">
          <div className="md:col-span-2">
            <Input
              id="productName"
              label="Nome do Produto (Ex: Pilsen, IPA)"
              type="text"
          ...
          <Select
            id="category"
            label="Categoria do Chopp"
            register={register("category")}
            error={errors.category?.message}
            disabled={isSavingItem}
            options={Object.values(StockCategory).map((cat) => ({
              id: cat,
              name: cat,
            }))}
            optionName="name"
          ...
          <Select
            id="unitLiters"
            label="Litragem por unidade"
            register={register("unitLiters", { valueAsNumber: true })}
            error={errors.unitLiters?.message}
            disabled={isSavingItem || isEditing}
            options={[
              { id: 10, name: "10 Litros" },
              { id: 20, name: "20 Litros" },
              { id: 30, name: "30 Litros" },
              { id: 50, name: "50 Litros" },
            ]}
            optionName="name"
            optionId="id"
          />
          <Input
            id="unitCount"
            label="Qtd. de unidades (barris)"
            type="number"
            register={register("unitCount", { valueAsNumber: true })}
          ...
          {!isEditing && totalQuantity > 0 && (
            <div className="md:col-span-2 bg-indigo-50 dark:bg-indigo-900 p-3 rounded-md text-center">
              <p className="text-sm font-medium text-indigo-800 dark:text-indigo-200">
                ...
                <span className="font-bold">{totalQuantity} Litros</span>
              </p>
            </div>
          )}
          ...
          <Input
            id="unitPrice"
            label="Preço Unitário (por barril)"
            type="number"
            step="0.01"
        ...
        <div className="mt-6 flex justify-end space-x-4">
          <Button
            ...
            variant="secondary"
            onClick={onCancel}
            disabled={isSavingItem || isSavingExit}
          >
            Fechar
          </Button>
          <Button
      ...
      {currentItem && (
        <div className="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
          ...
          {currentItem.status === StockStatus.CLOSED ? (
            <p className="text-sm text-red-600 bg-red-50 dark:bg-red-900 dark:text-red-200 p-3 rounded-md">
              Este item está encerrado e não pode ter novas saídas registradas.
            </p>
          ) : (
            <StockExitForm
              formMethods={exitFormMethods}
              onSave={onSaveExit}
