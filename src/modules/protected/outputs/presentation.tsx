...
export default function OutputsPresentation({
  ...
  return (
    <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-4 items-end">
            ...
            <div className="lg:col-span-3">
             ...
             <div className="lg:col-span-2">
                <label htmlFor="filter-type" className={labelBaseClasses}>Tipo</label>
                <select id="filter-type" value={filters.type} onChange={(e) => onFilterChange('type', e.target.value)} className={inputBaseClasses}>
      ...
      <Card>
        <div className="overflow-x-auto">
            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    {outputs.map(output => (
                        <tr key={output.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200">
                            <td className="px-6 py-4">{new Date(output.date).toLocaleDateString()}</td>
                            <td className="px-6 py-4">{output.category}</td>
                            <td className="px-6 py-4 capitalize">{output.type}</td>
                            <td className="px-6 py-4 capitalize">{output.paymentMethod}</td>
                            <td className="px-6 py-4 text-right font-medium text-red-500">
                                {output.value.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                            </td>
                            <td className="px-6 py-4 text-center">
                                <button onClick={() => onOpenModal(output)} className="font-medium text-indigo-600 dark:text-indigo-500 hover:underline mr-4">Editar</button>
                                <button onClick={() => onDeleteOutput(output.id)} className="font-medium text-red-600 dark:text-red-500 hover:underline">Excluir</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
                <tfoot>
                    <tr className="font-semibold text-gray-900 dark:text-white bg-gray-50 dark:bg-gray-700">
                        ...
                        <td className="px-6 py-3 text-base text-right">{totalOutputs.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</td>
                        <td></td>
                    </tr>
                </tfoot>
            </table>
      ...
      <Modal isOpen={isModalOpen} onClose={onCloseModal} title={editingOutput ? 'Editar Saída' : 'Nova Saída'}>
        <OutputForm
